...
.PHONY: deps smoke down logs health ready pycheck unittest test check \
	preflight validate-config validate-image repo-scan repo-clean repo-scan-json \
	maintenance maintenance-fix maintenance-json maintenance-all maintenance-all-json \
	maintenance-baseline bundle clean
