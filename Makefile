.PHONY: \
	smoke down logs health ready \
	pycheck unittest test check validate-config \
	repo-scan repo-clean maintenance maintenance-fix \
	bundle clean
...
unittest:
	python3 -m unittest egressd/test_supervisor.py scripts/test_repo_hygiene.py scripts/test_repo_maintenance.py

test:
	python3 -m unittest discover -s egressd -p "test_*.py"
	python3 -m unittest scripts/test_repo_hygiene.py scripts/test_repo_maintenance.py
