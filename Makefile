...
.PHONY: deps smoke down logs health ready pycheck unittest test check \
	preflight validate-config validate-image \
...
pycheck:
	$(PYTHON) -m py_compile \
		egressd/supervisor.py egressd/chain.py egressd/readiness.py egressd/preflight.py \
		egressd/test_supervisor.py egressd/test_supervisor_readiness.py \
		client/test_client.py exitserver/echo_server.py \
		funkydns-smoke/check_resolution.py funkydns-smoke/generate_cert.py funkydns-smoke/run_funkydns.py \
		tests/test_chain.py tests/test_preflight.py tests/test_hop_connectivity.py \
		scripts/repo_hygiene.py scripts/repo_maintenance.py

unittest:
	$(PYTHON) -m unittest \
		egressd/test_supervisor_readiness.py \
		egressd/test_supervisor.py \
		tests/test_readiness.py \
		tests/test_supervisor.py \
		tests/test_chain.py \
		tests/test_preflight.py \
		tests/test_hop_connectivity.py \
