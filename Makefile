...
preflight: validate-image
	$(PODMAN) run --rm \
		-e EGRESSD_VALIDATE_ONLY=1 \
		-e EGRESSD_PREFLIGHT_SKIP_BIN_CHECKS=1 \
		$(EGRESSD_IMAGE) \
		$(PYTHON) /opt/egressd/supervisor.py --check-config

validate-config: validate-image
	$(PODMAN) run --rm \
		-e EGRESSD_VALIDATE_ONLY=1 \
		$(EGRESSD_IMAGE) \
		$(PYTHON) /opt/egressd/supervisor.py
...
# First-party default scan; excludes third_party to avoid vendored TODO noise.
maintenance:
