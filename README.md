...
Run repository maintenance checks (unfinished markers + stray artifacts):

```bash
make maintenance
```

For automatic cleanup of removable clutter (untracked stray artifacts):
...
To include `third_party/FunkyDNS` in the scan explicitly, use:
