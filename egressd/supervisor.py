...
def _hop_health_signature(hops: List[Any], hop_statuses: Dict[str, Any]) -> Tuple[bool, ...]:
    """Return ordered per-hop health booleans for change detection."""
    return tuple(
        bool(hop_statuses.get(f"hop_{idx}", {}).get("ok", False))
        for idx in range(len(hops))
    )
...
def hop_health_loop(cfg: Dict[str, Any]) -> None:
    ...
    last_health_signature: Optional[Tuple[bool, ...]] = None
    first_run = True
    while not STOP_EVENT.is_set():
        checked_at = int(time.time())
        ...
        if first_run or current_signature != last_health_signature:
            print_chain_visual(cfg, statuses)
            last_health_signature = current_signature
            first_run = False
