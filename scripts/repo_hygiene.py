...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        # Gitlink files indicate legitimate submodule checkouts.
        if git_path.is_file():
            try:
...
def command_clean(
    ...
    remaining_untracked = list_git_paths(repo_root, ("ls-files", "--others", "--exclude-standard"))
    remaining_stray = classify_stray_paths(remaining_untracked, include_third_party=include_third_party)
    _, remaining_stale_untracked = find_stale_artifacts(
        tracked_paths=(),
        untracked_paths=remaining_untracked,
        include_third_party=include_third_party,
    )
    report["clean"] = {
