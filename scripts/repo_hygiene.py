...
DEFAULT_STALE_ARTIFACT_PATHS = frozenset(
    {
...
def find_stale_artifacts(
    ...
    configured = set(stale_artifact_paths)
    stale_tracked = sorted(path for path in configured if path in tracked_set)
    stale_untracked = sorted(path for path in configured if path in untracked_set)
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        metavar="PATH",
        help=(
            "add a stale artifact path relative to repo root. "
...
def build_stale_artifact_paths(extra_paths: Sequence[str]) -> set[str]:
    configured = set(DEFAULT_STALE_ARTIFACT_PATHS)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    stale_artifact_paths = build_stale_artifact_paths(args.stale_artifact)
