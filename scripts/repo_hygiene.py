...
def discover_embedded_git_repos(
    ...
    """Return embedded git repository paths relative to *repo_root*."""
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        if git_path == root_git:
            continue
        try:
            rel_parent = git_path.parent.relative_to(repo_root)
        except ValueError:
            continue
        rel_parent_path = rel_parent.as_posix()
        if not include_third_party and rel_parent_path.startswith(THIRD_PARTY_PREFIX):
            continue
        # Skip valid gitlink metadata files used by submodules.
        if git_path.is_file():
            try:
            ...
            if first_line.startswith("gitdir:"):
                continue
        embedded.append(rel_parent_path)
    return embedded
