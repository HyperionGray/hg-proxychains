...
def classify_stray_paths(
    ...
    stray: list[str] = []
    stale_names = {Path(p).name for p in STALE_ARTIFACT_PATHS}
    for rel_path in untracked_paths:
        ...
        if any(fnmatch.fnmatch(basename, pattern) for pattern in STRAY_FILE_PATTERNS):
            stray.append(rel_path)
            continue
        if rel_path in STALE_ARTIFACT_PATHS or basename in stale_names:
            stray.append(rel_path)
...
def gather_scan_state(
    ...
    return findings, stray, stale_tracked, stale_untracked, suppressed_markers
...
def command_scan(
    ...
    findings, stray, stale_tracked, stale_untracked, suppressed_markers = gather_scan_state(
        repo_root,
        include_third_party=include_third_party,
...
def command_clean(
    ...
    remaining = sorted(path for path in deletable if (repo_root / path).exists())
    clean_report = {
        "attempted_delete_paths": len(deletable),
    ...
    if json_output:
        ...
        if remaining:
            print(f"remaining removable paths: {len(remaining)}")
            for rel_path in remaining:
                print(f"  - {rel_path}")

    if findings or stale_tracked or remaining:
        return 1
    return 0
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help="include third_party/FunkyDNS markers and untracked paths",
    )
    parser.add_argument(
        "--baseline-file",
