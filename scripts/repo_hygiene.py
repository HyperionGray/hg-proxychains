...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Return nested repository roots that contain an embedded ``.git`` entry.

    The top-level repository ``.git`` is ignored. Gitlink files used by
    submodules (first line starts with ``gitdir:``) are also ignored.
    """
