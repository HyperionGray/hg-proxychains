...
def list_submodule_paths(repo_root: Path, submodule_rel: str, args: Sequence[str]) -> list[str]:
    ...
    if not submodule_root.exists():
        return []
    try:
        rel_paths = list_git_paths(submodule_root, args)
    except RuntimeError:
        return []
    return [f"{submodule_rel}/{path}" for path in rel_paths]
...
def find_unfinished_markers(
    ...
    for rel_path in tracked_paths:
        ...
        for idx, line in enumerate(text.splitlines(), start=1):
            ...
            if match:
                findings.append(MarkerFinding(rel_path, idx, match.group(1), line.strip()))
    return findings
...
def load_marker_baseline(repo_root: Path, baseline_path: str) -> set[tuple[str, str, str]]:
    ...
    entries = payload.get("unfinished_markers", [])
...
def collect_scan_state(
    ...
    baseline_path: str,
) -> tuple[list[MarkerFinding], list[str], list[str], list[str], int]:
    ...
    return findings, stray, stale_tracked, stale_untracked, suppressed
...
def command_scan(
    ...
    findings, stray, stale_tracked, stale_untracked, suppressed = collect_scan_state(
        repo_root,
        include_third_party=include_third_party,
        baseline_path=baseline_path,
    )
    report = build_scan_report(findings, stray, stale_tracked, stale_untracked, suppressed)
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    else:
        print_scan_results(findings, stray, stale_tracked, stale_untracked, suppressed)
...
def command_clean(
    ...
    post_findings, post_stray, post_stale_tracked, post_stale_untracked, post_suppressed = collect_scan_state(
        repo_root,
        include_third_party=include_third_party,
        baseline_path=baseline_path,
    )
    ...
    if json_output:
        ...
        print(f"deleted stray/stale untracked paths: {deleted}")
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="scan",
        help="scan for issues, clean removable clutter, or write marker baseline",
    )
