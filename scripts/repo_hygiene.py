...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Return relative parent paths for embedded git repositories.

    Root-level ``.git`` metadata is excluded. Gitlink files whose first line
    starts with ``gitdir:`` are treated as valid submodule metadata and skipped.
    """
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if git_path.is_file():
            try:
                ...
                # Treat unreadable metadata conservatively as suspicious.
...
def command_scan(
    ...
    report = build_scan_report(
        findings, stray, stale_tracked, stale_untracked, embedded_git_repos
    )
    if suppressed:
