...
def build_scan_report(
    ...
    return {
        ...
        "stale_artifacts": {
            "tracked": list(stale_tracked),
            "untracked": list(stale_untracked),
        },
...
def command_clean(
    ...
    stray_remaining = classify_stray_paths(post_untracked, include_third_party=include_third_party)
    _, stale_untracked_remaining = find_stale_artifacts(tracked, post_untracked)

    report["clean"] = {
        "deleted_stray_paths": deleted_stray,
        "deleted_stale_untracked_artifacts": deleted_stale_untracked,
        "remaining_stray_untracked_paths": len(stray_remaining),
        "remaining_stale_untracked_artifacts": len(stale_untracked_remaining),
    }
    if not json_output:
    ...
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or stale_tracked or embedded_git_repos or stray_remaining or stale_untracked_remaining else 0
