        ...
        if (funky_root / ".git").exists():
            try:
                for dep_path in list_git_paths(funky_root, list_args):
                    paths.add(str(Path(FUNKYDNS_PREFIX) / dep_path))
            except RuntimeError:
                pass
    return sorted(paths)
...
def print_scan_results(findings: Sequence[MarkerFinding], stray_paths: Sequence[str], suppressed_markers: int = 0) -> None:
...
def command_scan(
    ...
    report = build_scan_report(findings, stray, suppressed_markers=suppressed)
    if json_output:
...
def command_clean(
    ...
    if not json_output:
        print_scan_results(findings, stray, suppressed_markers=suppressed)
    deleted = delete_paths(repo_root, stray) if stray else 0
