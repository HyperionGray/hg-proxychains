...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Find embedded git repositories under repo_root.

    Ignores:
    - the repository root .git entry
    - gitlink files used by submodules/worktrees ("gitdir: ...")
    - third_party/* paths unless include_third_party=True
    """
...
def command_scan(
    ...
    stale_untracked_set = set(stale_untracked)
    if stale_untracked_set:
        stray = [path for path in stray if path not in stale_untracked_set]
