...
STALE_ARTIFACT_PATHS = set(STALE_UNTRACKED_NAMES)
...
def command_scan(
    ...
    *,
    ...
    findings_all = find_unfinished_markers(
        repo_root,
        tracked,
        include_third_party=include_third_party,
        excluded_paths={baseline_rel_path},
    )
    marker_baseline = load_marker_baseline(repo_root, baseline_path)
    findings, suppressed_markers = apply_marker_baseline(findings_all, marker_baseline)
    ...
    report["suppressed_unfinished_markers"] = suppressed_markers
    report["stale_tracked_artifacts"] = stale_tracked
    report["stale_untracked_artifacts"] = stale_untracked
    report["summary"]["total_issues"] = (
        len(findings) + len(stray) + len(stale_tracked) + len(stale_untracked)
    )
    if json_output:
...
def command_clean(
    ...
    *,
    ...
    removable = sorted(set(stray + stale_untracked))
    if removable:
