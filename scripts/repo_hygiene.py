...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        # Submodule worktrees use .git files with "gitdir:" pointers; these are expected.
        if git_path.is_file():
            try:
...
def command_clean(
    ...
    if stray:
        ...
        deletion_failures += len(stray) - deleted_stray
        if not json_output:
    ...
    if stale_untracked:
        ...
        deletion_failures += len(stale_untracked) - deleted_stale_untracked
        if not json_output:
    ...
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or stale_tracked or embedded_git_repos or deletion_failures else 0
