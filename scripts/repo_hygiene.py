...
ALLOWED_GITLINK_PATHS = frozenset({"third_party/FunkyDNS"})
...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Detect embedded git repositories outside approved gitlink paths."""
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        # Keep approved gitlink entries (for example the third-party submodule).
        if rel_parent in ALLOWED_GITLINK_PATHS and git_path.is_file():
            try:
...
def command_scan(
    ...
    embedded = discover_embedded_git_repos(
        repo_root,
        include_third_party=include_third_party,
    )
