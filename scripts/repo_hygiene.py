...
def _is_allowed_submodule_gitlink(repo_root: Path, git_entry: Path) -> bool:
    if not git_entry.is_file():
        return False
    try:
        rel_parent = git_entry.parent.relative_to(repo_root).as_posix()
    except ValueError:
        return False
    if rel_parent != "third_party/FunkyDNS":
        return False
    try:
        first_line = git_entry.read_text(encoding="utf-8", errors="ignore").split("\n", 1)[0]
    except OSError:
        return False
    return first_line.startswith("gitdir:")


def find_embedded_git_repositories(
    repo_root: Path,
    include_third_party: bool = False,
) -> list[str]:
    ...
    for git_entry in sorted(repo_root.rglob(".git")):
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        if _is_allowed_submodule_gitlink(repo_root, git_entry):
            continue
...
def build_scan_report(
    ...
    embedded_git_paths: Sequence[str],
) -> dict[str, object]:
    return {
        "unfinished_markers": [
            {
        ...
        "stale_tracked_paths": list(stale_tracked_paths),
        "stale_untracked_paths": list(stale_untracked_paths),
        "embedded_git_repositories": list(embedded_git_paths),
        "summary": {
            "unfinished_markers": len(findings),
            "stray_untracked_paths": len(stray_paths),
            "stale_tracked_paths": len(stale_tracked_paths),
            "stale_untracked_paths": len(stale_untracked_paths),
            "embedded_git_repositories": len(embedded_git_paths),
            "total_issues": (
                len(findings)
                + len(stray_paths)
                + len(stale_tracked_paths)
                + len(stale_untracked_paths)
                + len(embedded_git_paths)
            ),
        },
    }
...
def print_scan_results(
    ...
    if stale_untracked_paths:
        for rel_path in stale_untracked_paths:
            print(f"  - {rel_path}")
    print(f"embedded git repositories: {len(embedded_git_paths)}")
    if embedded_git_paths:
        for rel_path in embedded_git_paths:
            print(f"  - {rel_path}")
...
def command_scan(
    ...
    embedded_git = find_embedded_git_repositories(
        repo_root,
        include_third_party=include_third_party,
    )
    report = build_scan_report(findings, stray, stale_tracked, stale_untracked, embedded_git)
    if suppressed:
    ...
    if json_output:
        ...
        print_scan_results(findings, stray, stale_tracked, stale_untracked, embedded_git)
    return 1 if findings or stray or stale_tracked or stale_untracked or embedded_git else 0
...
def command_clean(
    ...
    if cleanup_targets:
        ...
        report["clean"] = {
            ...
            "requested_paths": len(cleanup_targets),
        }
        if not json_output:
            print(f"deleted cleanup paths: {deleted}/{len(cleanup_targets)}")
    else:
        report["clean"] = {"deleted_paths": 0, "requested_paths": 0}
        if not json_output:
            print("deleted cleanup paths: 0/0")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or stale_tracked or embedded_git else 0
