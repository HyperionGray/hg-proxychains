...
def discover_embedded_git_repos(
    ...
    """Return relative paths containing stray nested git repositories."""
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        # A gitlink file points to a submodule checkout and is expected.
        if git_path.is_file():
            try:
...
def command_clean(
    ...
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or embedded_git else 0
