...
def find_stale_artifacts(
    ...
    tracked_set = {path for path in tracked_paths if _include(path)}
    untracked_set = {path for path in untracked_paths if _include(path)}
...
def print_scan_results(report: dict[str, Any]) -> None:
    ...
    findings = report.get("unfinished_markers", [])
    stray_untracked_paths = report.get("stray_untracked_paths", [])
    ...
    print(f"unfinished markers: {summary.get('unfinished_markers', len(findings))}")
    suppressed = int(summary.get("suppressed_markers", 0))
    if suppressed:
    ...
    print(f"stray untracked files: {summary.get('stray_untracked_paths', len(stray_untracked_paths))}")
    if stray_untracked_paths:
        for rel_path in stray_untracked_paths:
            print(f"  - {rel_path}")

    print(f"stale tracked artifacts: {summary.get('stale_tracked_artifacts', len(stale_tracked))}")
    if stale_tracked:
        for rel_path in stale_tracked:
            print(f"  - {rel_path}")

    print(f"stale untracked artifacts: {summary.get('stale_untracked_artifacts', len(stale_untracked))}")
    if stale_untracked:
...
def report_has_issues(report: dict[str, Any]) -> bool:
    summary = report.get("summary", {})
    return int(summary.get("total_issues", 0)) > 0
...
def command_clean(
    ...
    removable_paths = set(initial["stray_untracked_paths"]) | set(initial["stale_untracked_artifacts"])
