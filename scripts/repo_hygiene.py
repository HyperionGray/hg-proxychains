...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Return unexpected nested git repository roots under *repo_root*.

    The repository root `.git` entry is excluded. Submodule gitlink files
    (regular files whose first line starts with ``gitdir:``) are also excluded.
    """
    root_git = repo_root / ROOT_GIT_DIR_NAME
    embedded: set[str] = set()
    for git_path in sorted(repo_root.rglob(ROOT_GIT_DIR_NAME)):
