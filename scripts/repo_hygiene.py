...
3) unexpected embedded git repositories
"""
...
ALLOWED_EMBEDDED_GIT_REPOS = frozenset({"third_party/FunkyDNS"})
...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in repo_root.rglob(".git"):
        ...
        rel_repo_dir = repo_dir.relative_to(repo_root).as_posix()
        if rel_repo_dir in ALLOWED_EMBEDDED_GIT_REPOS:
            continue
        if not include_third_party and rel_repo_dir.startswith(THIRD_PARTY_PREFIX):
            continue
        embedded.add(rel_repo_dir)
...
def build_scan_report(
    ...
    suppressed_baseline_markers: int = 0,
) -> dict[str, object]:
    return {
        "unfinished_markers": [
            {
        ...
        "summary": {
            ...
            "suppressed_baseline_markers": suppressed_baseline_markers,
            "total_issues": len(findings) + len(stray_paths) + len(embedded_git_repos),
        },
    }


def print_scan_results(findings: Sequence[MarkerFinding], stray_paths: Sequence[str], embedded_git_repos: Sequence[str]) -> None:
    ...
    if embedded_git_repos:
        for repo_path in embedded_git_repos:
            print(f"  - {repo_path}")
...
def command_scan(
    ...
    report = build_scan_report(
        ...
        stray,
        embedded_git_repos,
        suppressed_baseline_markers=suppressed,
    )
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    else:
        print_scan_results(findings, stray, embedded_git_repos)
    return 1 if findings or stray or embedded_git_repos else 0
...
def command_clean(
    ...
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or embedded_git_repos else 0
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help="include third_party/FunkyDNS markers/stray files and embedded-git checks under third_party",
    )
    parser.add_argument(
        "--baseline-file",
        default=BASELINE_DEFAULT_PATH,
        help=f"path to marker baseline JSON file (default: {BASELINE_DEFAULT_PATH})",
    )
