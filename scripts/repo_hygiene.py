...
3) known stale artifacts (tracked + untracked)
...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        if rel_parent == FUNKYDNS_SUBMODULE_PATH and is_gitlink(git_path):
            # Expected submodule marker for third_party/FunkyDNS.
            continue
...
def command_clean(
    ...
    if removable:
        ...
        requested = len(removable)
        failed = requested - deleted
        report["clean"] = {
            "requested_removals": requested,
            "deleted_removable_paths": deleted,
            "failed_removable_paths": failed,
        }
        if not json_output:
            print(f"deleted removable paths: {deleted}/{requested}")
    else:
        failed = 0
        report["clean"] = {
            "requested_removals": 0,
            "deleted_removable_paths": 0,
            "failed_removable_paths": 0,
        }
        if not json_output:
            print("deleted removable paths: 0/0")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or stale_tracked or embedded_git_repos or failed else 0
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "include all of third_party/ (e.g. third_party/FunkyDNS) in marker, stray, "
            "stale-artifact, and embedded-git scanning (default: false)"
        ),
    )
    parser.add_argument(
        "--baseline-file",
