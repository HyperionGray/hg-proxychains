...
def find_stale_artifacts(
    ...
    def include_path(rel_path: str) -> bool:
        return include_third_party or not rel_path.startswith(FUNKYDNS_PREFIX)

    stale_tracked = sorted(
        path for path in STALE_ARTIFACT_PATHS if path in tracked_set and include_path(path)
    )
    stale_untracked = sorted(
        path for path in STALE_ARTIFACT_PATHS if path in untracked_set and include_path(path)
    )
...
def command_scan(
    ...
    tracked = collect_git_paths(
        repo_root, ("ls-files",), include_third_party=include_third_party
    )
    ...
    stray_untracked = classify_stray_paths(
        untracked, include_third_party=include_third_party
    )
...
def command_clean(
    ...
    removable_paths = sorted(set(stray_untracked) | set(stale_untracked))
    deleted = delete_paths(repo_root, removable_paths) if removable_paths else 0
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help="include third_party/FunkyDNS when scanning markers and stray artifacts",
    )
    parser.add_argument(
        "--baseline-file",
