...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Return unexpected embedded git repository paths."""
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if not git_path.is_dir() and not git_path.is_file():
            continue
        try:
            rel_git = git_path.relative_to(repo_root).as_posix()
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        if git_path.is_file():
            if rel_git in ALLOWED_SUBMODULE_GITLINKS:
                continue
            try:
