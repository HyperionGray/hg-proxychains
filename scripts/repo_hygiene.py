...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Find nested git repositories outside explicitly allowed paths."""
    ...
    for current_root, dirnames, filenames in os.walk(repo_root):
        ...
        # Skip third_party/ entirely in first-party mode to avoid dependency noise.
        if not include_third_party and rel_path == "" and "third_party" in dirnames:
            dirnames.remove("third_party")

        # Root .git is expected and should never be reported.
        if ".git" in dirnames:
            if rel_path and rel_path not in ALLOWED_EMBEDDED_GIT_PATHS:
                embedded.add(rel_path)
            dirnames.remove(".git")

        if ".git" in filenames and rel_path:
