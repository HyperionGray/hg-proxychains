...
2) common stray untracked files (editor backups, Python caches, temp files)
3) known stale artifacts (tracked and untracked)
...
def find_stale_artifacts(
    ...
    if not include_third_party:
        tracked_set = {path for path in tracked_set if not path.startswith(THIRD_PARTY_PREFIX)}
        untracked_set = {path for path in untracked_set if not path.startswith(THIRD_PARTY_PREFIX)}
    ...
    stale_untracked = sorted(
        ...
        if path in STALE_ARTIFACT_PATHS or Path(path).name in STALE_UNTRACKED_NAMES
    )
...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Return parent paths of nested .git directories/files under repo_root."""
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        try:
            ...
            continue
        if rel_parent == ".git" or rel_parent.startswith(".git/"):
            continue
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        # Gitlink files mark legitimate submodule checkouts.
        if git_path.is_file():
            try:
...
def build_scan_report(
    ...
    return {
        ...
        "stale_artifacts_tracked": list(stale_tracked_paths),
        "stale_artifacts_untracked": list(stale_untracked_paths),
...
def command_clean(
    ...
    cleanup_targets = sorted(set(stray) | set(stale_untracked))
    if cleanup_targets:
        deleted = delete_paths(repo_root, cleanup_targets)
        remaining_cleanup_paths = sorted(path for path in cleanup_targets if (repo_root / path).exists())
        report["clean"] = {
            "deleted_cleanup_paths": deleted,
            "requested_cleanup_paths": len(cleanup_targets),
            "remaining_cleanup_paths": remaining_cleanup_paths,
        }
        if not json_output:
            print(f"deleted cleanup paths: {deleted}")
            if remaining_cleanup_paths:
                print(f"remaining cleanup paths: {len(remaining_cleanup_paths)}")
                for rel_path in remaining_cleanup_paths:
                    print(f"  - {rel_path}")
    else:
        report["clean"] = {
            "deleted_cleanup_paths": 0,
            "requested_cleanup_paths": 0,
    ...
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    remaining_cleanup_paths = report["clean"].get("remaining_cleanup_paths", [])
    return 1 if findings or stale_tracked or embedded_git_repos or remaining_cleanup_paths else 0
