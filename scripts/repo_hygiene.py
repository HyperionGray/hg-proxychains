...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        if git_path == root_git:
            continue
        try:
            rel_git_path = git_path.relative_to(repo_root).as_posix()
        except ValueError:
            continue
        if rel_git_path.startswith(".git/"):
            continue
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        # A gitdir file is expected for proper submodules; treat it as safe.
        if git_path.is_file():
            try:
...
def command_clean(
    ...
    if embedded_git_repos:
        report["clean"]["embedded_git_repositories_removed"] = 0
        if not json_output:
            print("embedded git repositories removed: 0 (manual cleanup required)")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
