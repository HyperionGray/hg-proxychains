...
def print_scan_results(report: dict[str, Any]) -> None:
    ...
    suppressed_markers = report["summary"]["suppressed_markers"]
...
def scan_repo(
    ...
    return build_scan_report(filtered_findings, stray, stale_tracked, stale_untracked, suppressed)


def report_has_issues(report: dict[str, Any]) -> bool:
    return bool(
        report["unfinished_markers"]
        or report["stray_untracked_paths"]
        or report["stale_tracked_artifacts"]
        or report["stale_untracked_artifacts"]
    )
...
def command_scan(
    ...
    report = scan_repo(repo_root, include_third_party=include_third_party, baseline_path=baseline_path)
    if json_output:
...
def command_clean(
    ...
    report = scan_repo(repo_root, include_third_party=include_third_party, baseline_path=baseline_path)
    to_delete = list(report["stray_untracked_paths"]) + list(report["stale_untracked_artifacts"])
    deleted = delete_paths(repo_root, to_delete) if to_delete else 0

    report["clean"] = {
        "candidate_paths": to_delete,
    ...
    if json_output:
        ...
        print_scan_results(report)
        print(f"deleted removable stray paths: {deleted}")

    # clean mode can auto-remove stray+stale untracked paths, but unfinished markers and
    # stale tracked artifacts still require manual changes.
    return 1 if report["unfinished_markers"] or report["stale_tracked_artifacts"] else 0
