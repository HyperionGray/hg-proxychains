...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Return embedded git repositories outside allowed submodule gitlinks."""
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        # Allow submodule gitlink files ("gitdir: ...") such as third_party/FunkyDNS/.git.
        if git_path.is_file():
            try:
...
def build_scan_report(
    ...
    embedded_git_repos: Sequence[str] | None = None,
    stale_tracked_paths: Sequence[str] | None = None,
    stale_untracked_paths: Sequence[str] | None = None,
) -> dict[str, object]:
    embedded = list(embedded_git_repos or [])
    stale_tracked = list(stale_tracked_paths or [])
    stale_untracked = list(stale_untracked_paths or [])
    return {
        "unfinished_markers": [
            {
                "path": finding.path,
        ...
        "stale_tracked_paths": stale_tracked,
        "stale_untracked_paths": stale_untracked,
        "summary": {
            "unfinished_markers": len(findings),
            "stray_untracked_paths": len(stray_paths),
            "embedded_git_repos": len(embedded),
...
def command_clean(
    ...
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or embedded_git_repos or stale_tracked else 0
