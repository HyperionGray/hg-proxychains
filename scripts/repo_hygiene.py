...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Find nested git repositories that are not legitimate gitlinks."""
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        if git_path == root_git:
            continue
        try:
            rel_repo_path = git_path.parent.relative_to(repo_root).as_posix()
        except ValueError:
            continue
        if not include_third_party and rel_repo_path.startswith(THIRD_PARTY_PREFIX):
            continue
        if git_path.is_file():
            try:
                first_line = git_path.read_text(encoding="utf-8", errors="ignore").splitlines()
            except OSError:
                first_line = []
            if first_line and first_line[0].startswith("gitdir:"):
                continue
        embedded.append(rel_repo_path)
    return embedded
...
def command_scan(
    ...
    return 1 if findings or stray or embedded_git_repos or stale_tracked or stale_untracked else 0
...
def command_clean(
    ...
    if removable_paths:
        ...
        report["clean"] = {"deleted_removable_paths": deleted}
        if not json_output:
            print(f"deleted removable paths: {deleted}")
    else:
        report["clean"] = {"deleted_removable_paths": 0}
        if not json_output:
            print("deleted removable paths: 0")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    remaining_removable = [path for path in removable_paths if (repo_root / path).exists()]
    return 1 if findings or embedded_git_repos or stale_tracked or remaining_removable else 0
