...
def find_stale_artifacts(
    ...
    stale_tracked = sorted(
        ...
        if path in STALE_ARTIFACT_PATHS
        and (include_third_party or not path.startswith(THIRD_PARTY_PREFIX))
    )
...
def is_gitlink_file(git_path: Path) -> bool:
...
def find_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        if git_path == root_git:
            continue
        if is_gitlink_file(git_path):
            continue
        try:
            rel = git_path.parent.relative_to(repo_root).as_posix()
        except ValueError:
            continue
...
def command_clean(
    ...
    if removable_paths:
        ...
        expected_deletes = len(removable_paths)
        failed_deletes = max(expected_deletes - deleted, 0)
        report["clean"] = {
            "deleted_removable_paths": deleted,
            "failed_removable_paths": failed_deletes,
        }
        if not json_output:
            print(f"deleted removable paths: {deleted}")
            if failed_deletes:
                print(f"failed removable paths: {failed_deletes}")
    else:
        report["clean"] = {"deleted_removable_paths": 0, "failed_removable_paths": 0}
        if not json_output:
    ...
    return (
        1
        if findings or stale_tracked or embedded_git_repos or report["clean"]["failed_removable_paths"]
        else 0
    )
