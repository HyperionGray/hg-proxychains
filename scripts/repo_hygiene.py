...
def find_stale_artifacts(
    ...
    stale_tracked = sorted(path for path in STALE_ARTIFACT_PATHS if path in tracked_set and include_path(path))
    stale_untracked = sorted(path for path in STALE_ARTIFACT_PATHS if path in untracked_set and include_path(path))
...
def command_clean(
    ...
    if stale_untracked:
        deleted_stale = delete_paths(repo_root, stale_untracked)
        if not json_output:
            print(f"deleted stale untracked artifacts: {deleted_stale}")
    if not stray and not json_output:
        print("deleted stray paths: 0")
    if not stale_untracked and not json_output:
        print("deleted stale untracked artifacts: 0")
    report["clean"] = {
        "deleted_stray_paths": deleted_stray,
        "deleted_stale_untracked_artifacts": deleted_stale,
        "deleted_paths": deleted_stray + deleted_stale,
    }
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
