...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Discover unexpected nested git repositories.

    This reports directories containing a `.git` entry, excluding:
    - the root repository `.git`
    - gitlink `.git` files used by legitimate submodule checkouts
    """
...
def command_clean(
    ...
    if not json_output:
        ...
        print(f"deleted stale untracked paths: {deleted_stale_untracked}")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))

    remaining_stray = max(0, len(stray) - deleted_stray)
    remaining_stale_untracked = max(0, len(stale_untracked) - deleted_stale_untracked)
    return 1 if findings or stale_tracked or embedded_git_repos or remaining_stray or remaining_stale_untracked else 0
