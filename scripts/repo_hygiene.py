...
# These are exact repo-relative paths.
...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Report unexpected nested git repositories.

    The root .git is ignored. Gitlink files are treated as legitimate submodule
    metadata and ignored as well.
    """
...
def command_clean(
    ...
    unresolved_cleanup_paths: list[str] = []
    if cleanup_targets:
        deleted = delete_paths(repo_root, cleanup_targets)
        unresolved_cleanup_paths = [path for path in cleanup_targets if (repo_root / path).exists()]
        report["clean"] = {
            "deleted_paths": deleted,
            "cleanup_target_paths": cleanup_targets,
            "unresolved_cleanup_paths": unresolved_cleanup_paths,
        }
        if not json_output:
        ...
        report["clean"] = {
            ...
            "unresolved_cleanup_paths": [],
        }
        if not json_output:
            print("deleted cleanup paths: 0")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or stale_tracked or embedded_git_repos or unresolved_cleanup_paths else 0
