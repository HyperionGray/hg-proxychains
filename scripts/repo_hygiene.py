...
def detect_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Return unexpected embedded git repositories under *repo_root*.

    The root-level ``.git`` entry and submodule gitlink files (``gitdir:``)
    are excluded. By default, anything under ``third_party/`` is skipped.
    """
...
def command_scan(
    ...
    embedded = detect_embedded_git_repos(repo_root, include_third_party=include_third_party)
...
def command_clean(
    ...
    if stray:
        ...
        remaining = [rel_path for rel_path in stray if (repo_root / rel_path).exists()]
        report["clean"] = {"deleted_stray_paths": deleted}
        if remaining:
            report["clean"]["remaining_stray_paths"] = remaining
        if not json_output:
            print(f"deleted stray paths: {deleted}")
            if remaining:
                print(f"remaining stray paths: {len(remaining)}")
    ...
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or embedded or remaining else 0
