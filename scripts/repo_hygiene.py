...
3) known stale artifact paths (tracked or untracked)
"""
...
def find_stale_artifacts(
    ...
    def eligible(path: str) -> bool:
        return include_third_party or not path.startswith(THIRD_PARTY_PREFIX)

    tracked_set = set(tracked_paths)
    untracked_set = set(untracked_paths)
    stale_tracked = sorted(path for path in STALE_ARTIFACT_PATHS if path in tracked_set and eligible(path))
    stale_untracked = sorted(path for path in STALE_ARTIFACT_PATHS if path in untracked_set and eligible(path))
...
def build_scan_report(
    ...
    return {
        ...
        "stale_artifacts": {
            ...
            "tracked_count": len(stale_tracked_paths),
            "untracked_count": len(stale_untracked_paths),
        },
        "summary": {
            ...
            "total_issues": len(findings) + len(stray_paths) + len(stale_tracked_paths) + len(stale_untracked_paths),
        },
    }
...
def command_scan(
    ...
    report = build_scan_report(findings, stray, stale_tracked, stale_untracked)
    if suppressed:
    ...
    if json_output:
        ...
        print_scan_results(findings, stray, stale_tracked, stale_untracked)
    return 1 if findings or stray or stale_tracked or stale_untracked else 0
...
def command_clean(
    ...
    if stray or stale_untracked:
        ...
        report["clean"] = {
            ...
            "deleted_stale_untracked_paths": deleted_stale_untracked,
        }
        if not json_output:
            print(f"deleted stray paths: {deleted_stray}")
            print(f"deleted stale untracked artifact paths: {deleted_stale_untracked}")
    else:
        report["clean"] = {
            "deleted_stray_paths": 0,
            "deleted_stale_untracked_paths": 0,
        }
        if not json_output:
            print("deleted stray paths: 0")
            print("deleted stale untracked artifact paths: 0")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or stale_tracked else 0
