...
def collect_git_paths(
    ...
    if include_third_party:
        paths.update(list_submodule_paths(repo_root, THIRD_PARTY_PREFIX.rstrip("/"), list_args))
    return sorted(paths)
...
def _resolve_baseline_path(repo_root: Path, baseline_path: str) -> Path:
...
def _baseline_excluded_path(repo_root: Path, baseline_path: str) -> set[str]:
    candidate = _resolve_baseline_path(repo_root, baseline_path)
    try:
...
def build_scan_report(
    ...
    *,
    ...
    path_issues = set(stray_paths) | set(stale_tracked) | set(stale_untracked)
    return {
        "unfinished_markers": [
            {
                "path": finding.path,
        ...
        "summary": {
            ...
            "total_issues": len(findings) + len(path_issues),
        },
    }
...
def command_scan(
    ...
    *,
    ...
    findings = find_unfinished_markers(
        ...
        tracked,
        include_third_party=include_third_party,
        excluded_paths=_baseline_excluded_path(repo_root, baseline_path),
    )
...
def command_clean(
    ...
    *,
    ...
    untracked_before = collect_git_paths(
        repo_root,
        ("ls-files", "--others", "--exclude-standard"),
        include_third_party=include_third_party,
    )
    stray_before = classify_stray_paths(untracked_before, include_third_party=include_third_party)
    _, stale_untracked_before = find_stale_artifacts([], untracked_before)
    delete_candidates = sorted(set(stray_before) | set(stale_untracked_before))
    deleted = delete_paths(repo_root, delete_candidates) if delete_candidates else 0
    ...
    stale_tracked, stale_untracked_after = find_stale_artifacts(tracked, untracked_after)
...
def command_baseline(
    ...
    *,
    ...
    target = _resolve_baseline_path(repo_root, baseline_path)
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    try:
        rendered_target = target.relative_to(repo_root)
    except ValueError:
        rendered_target = target
    print(f"wrote baseline entries: {len(findings)} -> {rendered_target}")
    return 0
