...
def find_stale_artifacts(
    ...
    for path in sorted(STALE_ARTIFACT_PATHS):
        if path.startswith(THIRD_PARTY_PREFIX) and not include_third_party:
            continue
        if path in tracked_set:
            stale_tracked.append(path)
        if path in untracked_set:
            stale_untracked.append(path)
...
def collect_scan_inputs(repo_root: Path, include_third_party: bool) -> tuple[list[str], list[str]]:
...
def command_scan(
    ...
    *,
    ...
    tracked, untracked = collect_scan_inputs(repo_root, include_third_party=include_third_party)
    baseline = load_marker_baseline(repo_root, baseline_path)
    baseline_rel_path = Path(baseline_path).as_posix()
    findings_raw = find_unfinished_markers(
        repo_root,
        tracked,
        include_third_party=include_third_party,
        excluded_paths={baseline_rel_path},
    )
    findings, suppressed = apply_marker_baseline(findings_raw, baseline)
...
def command_clean(
    ...
    *,
    ...
    cleanup_candidates = sorted(set(stray + stale_untracked))
    deleted = delete_paths(repo_root, cleanup_candidates)

    _, untracked_after = collect_scan_inputs(repo_root, include_third_party=include_third_party)
    ...
    report["clean"] = {
        "requested_deletions": len(cleanup_candidates),
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help="include third_party/FunkyDNS in marker and stray-path scanning",
    )
    parser.add_argument(
        "--baseline-file",
