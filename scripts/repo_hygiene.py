...
def find_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Find embedded git repositories, excluding approved submodule gitlinks."""
    root_git = repo_root / ".git"
    repos: set[str] = set()
    for git_entry in sorted(repo_root.rglob(".git")):
        if git_entry == root_git:
            continue
        repo_dir = git_entry.parent
        try:
            rel_repo = repo_dir.relative_to(repo_root).as_posix()
        except ValueError:
            continue
        if rel_repo in ALLOWED_EMBEDDED_GIT_PATHS:
            continue
        if not include_third_party and rel_repo.startswith(THIRD_PARTY_PREFIX):
            continue
        if git_entry.is_file():
            try:
                first_line = git_entry.read_text(encoding="utf-8", errors="ignore").splitlines()
            except OSError:
                first_line = []
