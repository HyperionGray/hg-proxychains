...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Return relative parent paths containing unexpected nested .git entries.

    The root-level repository metadata is ignored. Submodule gitlink files
    (text files whose first line starts with ``gitdir:``) are treated as
...
def print_scan_results(
    ...
    if stray_paths:
        for rel_path in stray_paths:
            print(f"  - {rel_path}")
    print(f"stale tracked paths: {len(stale_tracked_paths)}")
    if stale_tracked_paths:
        for rel_path in stale_tracked_paths:
            print(f"  - {rel_path}")
    print(f"stale untracked paths: {len(stale_untracked_paths)}")
...
def command_scan(
    ...
    embedded_git_repositories = discover_embedded_git_repos(
        repo_root,
        include_third_party=include_third_party,
    )
...
def command_clean(
    ...
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or stale_tracked or embedded_git_repositories else 0
