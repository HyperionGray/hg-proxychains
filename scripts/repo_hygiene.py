...
def is_gitlink_file(path: Path) -> bool:
    if not path.is_file():
        return False
    try:
...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        # Submodules appear as gitlink files and are expected.
...
def command_clean(
    ...
    if cleanup_candidates:
        ...
        remaining = len(cleanup_candidates) - deleted
        report["clean"] = {
            "attempted_cleanup_paths": len(cleanup_candidates),
            "deleted_cleanup_paths": deleted,
            "remaining_cleanup_paths": remaining,
        }
        if not json_output:
            print(f"deleted cleanup paths: {deleted}")
    else:
        report["clean"] = {
            "attempted_cleanup_paths": 0,
            "deleted_cleanup_paths": 0,
            "remaining_cleanup_paths": 0,
        }
        if not json_output:
