...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        # Gitlink files indicate submodule checkouts and are not embedded repos.
        if git_path.is_file():
            try:
...
def command_clean(
    ...
    if removable_paths:
        ...
        failed = len(removable_paths) - deleted
        report["clean"] = {
            "attempted_path_deletions": len(removable_paths),
            "deleted_path_deletions": deleted,
            "failed_path_deletions": failed,
            # Retained for compatibility with older automation parsers.
            "deleted_stray_paths": deleted,
        }
        if not json_output:
            print(f"attempted path deletions: {len(removable_paths)}")
            print(f"deleted path deletions: {deleted}")
            print(f"failed path deletions: {failed}")
    else:
        report["clean"] = {
            "attempted_path_deletions": 0,
            "deleted_path_deletions": 0,
            "failed_path_deletions": 0,
            "deleted_stray_paths": 0,
        }
        if not json_output:
            print("attempted path deletions: 0")
            print("deleted path deletions: 0")
            print("failed path deletions: 0")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    clean_report = report.get("clean", {})
    failed_deletions = 0
    if isinstance(clean_report, dict):
        failed_value = clean_report.get("failed_path_deletions", 0)
        if isinstance(failed_value, int):
            failed_deletions = failed_value
    return 1 if findings or stale_tracked or embedded_git_repos or failed_deletions > 0 else 0
