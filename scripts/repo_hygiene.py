...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    """Discover nested git repositories relative to repo root.

    Returns parent paths containing a nested ``.git`` marker. The root
    repository marker is ignored. Gitlink files (``.git`` text files whose
    first line starts with ``gitdir:``) are treated as legitimate submodule or
    worktree markers and skipped.
    """

    root_git = repo_root / GIT_DIR_NAME
    embedded: set[str] = set()
    for git_path in sorted(repo_root.rglob(GIT_DIR_NAME)):
        ...
        if git_path.is_file():
            try:
                first_line = git_path.read_text(encoding="utf-8", errors="ignore").splitlines()[0]
            except IndexError:
                first_line = ""
            except OSError:
                first_line = ""
...
def command_scan(
    ...
    report = build_scan_report(findings, stray, embedded_git_repos)
    if suppressed:
