...
ALLOWED_EMBEDDED_GITLINK_PATHS: frozenset[str] = frozenset(
    {
...
def find_stale_artifacts(
    ...
    skip_untracked_paths: set[str] | None = None,
) -> tuple[list[str], list[str]]:
    tracked_set = set(tracked_paths)
    untracked_set = set(untracked_paths)
    skip_untracked = skip_untracked_paths or set()
    stale_tracked = sorted(path for path in STALE_ARTIFACT_PATHS if path in tracked_set)
    stale_untracked = sorted(
        path for path in STALE_ARTIFACT_PATHS if path in untracked_set and path not in skip_untracked
    )
...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        # Treat known submodule gitlink files as expected.
        if git_path.is_file() and rel_parent in ALLOWED_EMBEDDED_GITLINK_PATHS:
            try:
...
def command_scan(
    ...
    stale_tracked, stale_untracked = find_stale_artifacts(
        tracked,
        untracked,
        skip_untracked_paths=set(stray),
    )
