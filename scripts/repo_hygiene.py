...
3) known stale artifacts that should not remain in the repository
"""
...
def command_scan(
    ...
    *,
    ...
    report = build_scan_report(findings, stray, stale_tracked, stale_untracked, suppressed_markers=suppressed_markers)
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    else:
        print_scan_results(findings, stray, stale_tracked, stale_untracked, suppressed_markers=suppressed_markers)
...
def command_clean(
    ...
    *,
    ...
    if not json_output:
        print_scan_results(findings, stray, stale_tracked, stale_untracked, suppressed_markers=suppressed_markers)
    removable = list(dict.fromkeys([*stray, *stale_untracked]))
    deleted = delete_paths(repo_root, removable) if removable else 0
    report["clean"] = {
        "deleted_paths": deleted,
        "requested_paths": len(removable),
    }
    if not json_output:
    ...
    remaining_removable = max(0, len(removable) - deleted)
    return 1 if findings or stale_tracked or remaining_removable else 0
