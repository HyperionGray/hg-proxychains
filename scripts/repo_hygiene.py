...
ALLOWED_EMBEDDED_GIT_PATHS = {f"{FUNKYDNS_PREFIX}.git"}
STRAY_FILE_PATTERNS = (
    "*~",
    "*.bak",
    "*.tmp",
...
def find_embedded_git_paths(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(repo_root, topdown=True):
        ...
        if not include_third_party and rel_prefix.startswith(FUNKYDNS_PREFIX):
            dirnames[:] = []
            continue

        if ".git" in dirnames:
            candidate = f"{rel_prefix}.git" if rel_prefix else ".git"
            if candidate not in ALLOWED_EMBEDDED_GIT_PATHS and candidate != ".git":
...
def collect_scan_state(
    ...
    embedded_git_paths = find_embedded_git_paths(repo_root, include_third_party=include_third_party)
    return ScanState(
        findings=findings,
        stray_paths=stray,
...
def has_issues(state: ScanState) -> bool:
    return bool(
        state.findings
        or state.stray_paths
        or state.stale_tracked
        or state.stale_untracked
        or state.embedded_git_paths
    )
...
def build_scan_report(state: ScanState) -> dict[str, Any]:
    return {
        ...
        "embedded_git_paths": list(state.embedded_git_paths),
        "summary": {
            "unfinished_markers": len(state.findings),
            "unfinished_markers_suppressed_by_baseline": state.suppressed_markers,
            ...
            "embedded_git_paths": len(state.embedded_git_paths),
            "total_issues": (
                len(state.findings)
                + len(state.stray_paths)
                + len(state.stale_tracked)
                + len(state.stale_untracked)
                + len(state.embedded_git_paths)
            ),
        },
    }
...
def print_scan_results(state: ScanState) -> None:
    ...
    print(f"stray untracked paths: {len(state.stray_paths)}")
    ...
    print(f"embedded git paths: {len(state.embedded_git_paths)}")
    for rel_path in state.embedded_git_paths:
        print(f"  - {rel_path}")
...
def command_scan(
    ...
    if json_output:
        print(json.dumps(build_scan_report(state), indent=2, sort_keys=True))
    else:
...
def command_clean(
    ...
    pre_clean = collect_scan_state(repo_root, include_third_party=include_third_party, baseline_path=baseline_path)
    deletion_candidates = sorted(set(pre_clean.stray_paths + pre_clean.stale_untracked))
    deleted = delete_paths(repo_root, deletion_candidates)
    post_clean = collect_scan_state(repo_root, include_third_party=include_third_party, baseline_path=baseline_path)

    if json_output:
        payload = {
            "pre_clean": build_scan_report(pre_clean),
            "clean": {
                "requested_paths": len(deletion_candidates),
                "deleted_paths": deleted,
            },
            "post_clean": build_scan_report(post_clean),
        }
        print(json.dumps(payload, indent=2, sort_keys=True))
    else:
        print_scan_results(pre_clean)
        print(f"deleted removable paths: {deleted}")
    return 1 if has_issues(post_clean) else 0
...
def command_baseline(
    ...
    serialized_findings.sort(key=lambda item: (item["path"], item["marker"], item["line"]))
    payload = {"unfinished_markers": serialized_findings}
    ...
    if json_output:
        print(
            json.dumps(
                {
                    "baseline_file": target.relative_to(repo_root).as_posix(),
                    "entries_written": len(serialized_findings),
                },
                indent=2,
                sort_keys=True,
            )
        ...
        print(f"wrote baseline entries: {len(serialized_findings)} -> {target.relative_to(repo_root)}")
    return 0
