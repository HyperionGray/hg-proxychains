...
4) embedded git repositories outside approved locations
"""
...
ALLOWED_EMBEDDED_GIT_REPO_PATHS: frozenset[str] = frozenset(
    {
...
def discover_embedded_git_repos(repo_root: Path, include_third_party: bool = False) -> list[str]:
    ...
    for git_path in sorted(repo_root.rglob(".git")):
        ...
        try:
            ...
            continue
        if rel_parent in ALLOWED_EMBEDDED_GIT_REPO_PATHS:
            continue
        if not include_third_party and rel_parent.startswith(THIRD_PARTY_PREFIX):
            continue
        if git_path.is_file():
            try:
                lines = git_path.read_text(encoding="utf-8", errors="ignore").splitlines()
                first_line = lines[0] if lines else ""
                if first_line.startswith("gitdir:"):
                    continue
            except OSError:
                pass
        discovered.add(rel_parent)
...
def command_scan(
    ...
    embedded_git = discover_embedded_git_repos(repo_root, include_third_party=include_third_party)
...
def command_clean(
    ...
    if stale_untracked:
        ...
        if not json_output:
            print("deleted stale untracked artifacts: 0")
    refreshed_untracked = list_git_paths(repo_root, ("ls-files", "--others", "--exclude-standard"))
    remaining_stray = classify_stray_paths(refreshed_untracked, include_third_party=include_third_party)
    _, remaining_stale_untracked = find_stale_artifacts(tracked, refreshed_untracked)
    report["clean"] = {
        "deleted_stray_paths": deleted_stray,
        "deleted_stale_untracked_artifacts": deleted_stale_untracked,
        "remaining_stray_untracked_paths": len(remaining_stray),
        "remaining_stale_untracked_artifacts": len(remaining_stale_untracked),
    }
    if not json_output:
        print(f"remaining stray paths: {len(remaining_stray)}")
        print(f"remaining stale untracked artifacts: {len(remaining_stale_untracked)}")
    if json_output:
        print(json.dumps(report, indent=2, sort_keys=True))
    return 1 if findings or stale_tracked or embedded_git or remaining_stray or remaining_stale_untracked else 0
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "include all of third_party/ (for example third_party/FunkyDNS) "
            "in marker, stray-file, stale-artifact, and embedded-git scanning "
            "(default: false)"
        ),
    )
    parser.add_argument(
