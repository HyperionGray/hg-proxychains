...
class RepoHygieneTests(unittest.TestCase):
    ...
    def test_classify_stray_paths_detects_backups_and_caches(self) -> None:
        ...
        self.assertNotIn("egressd-starter.tar.gz", stray_with_third_party)
    ...
    def test_find_embedded_git_repositories_skips_allowed_submodule_gitlink(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / ".git").mkdir()
            ...
            nested = root / "tools" / "scratch" / ".git"
            nested.mkdir(parents=True, exist_ok=True)

            found = repo_hygiene.find_embedded_git_repositories(root, include_third_party=True)

        self.assertEqual(found, ["tools/scratch"])

    def test_find_embedded_git_repositories_skips_third_party_unless_enabled(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / ".git").mkdir()
            nested = root / "third_party" / "FunkyDNS" / "nested" / ".git"
            nested.mkdir(parents=True, exist_ok=True)

            found_default = repo_hygiene.find_embedded_git_repositories(root, include_third_party=False)
            found_with_third_party = repo_hygiene.find_embedded_git_repositories(root, include_third_party=True)

        self.assertEqual(found_default, [])
        self.assertEqual(found_with_third_party, ["third_party/FunkyDNS/nested"])
