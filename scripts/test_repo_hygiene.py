...
class RepoHygieneTests(unittest.TestCase):
    ...
    def test_find_stale_artifacts_skips_third_party_by_default(self) -> None:
        tracked = []
        untracked = ["third_party/vendor/egressd-starter.tar.gz"]
        ...
        self.assertEqual(stale_tracked_all, [])
        self.assertEqual(stale_untracked_all, ["third_party/vendor/egressd-starter.tar.gz"])
    ...
    def test_discover_embedded_git_repos_respects_gitlinks_and_scope(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            nested_first_party = root / "scratch" / ".git"
            nested_first_party.mkdir(parents=True, exist_ok=True)
            nested_third_party = root / "third_party" / "vendor" / ".git"
            nested_third_party.mkdir(parents=True, exist_ok=True)

            default_scan = repo_hygiene.discover_embedded_git_repos(
                root,
                include_third_party=False,
            )
            full_scan = repo_hygiene.discover_embedded_git_repos(
                root,
                include_third_party=True,
            )

        self.assertEqual(default_scan, ["scratch"])
        self.assertEqual(full_scan, ["scratch", "third_party/vendor"])
