...
class RepoHygieneTests(unittest.TestCase):
    ...
    def test_discover_embedded_git_repos_skips_root_and_gitlinks(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            submodule_gitlink.write_text("gitdir: ../../.git/modules/third_party/FunkyDNS\n", encoding="utf-8")
            nested_git = root / "scratch" / ".git"
            nested_git.mkdir(parents=True, exist_ok=True)

            embedded = repo_hygiene.discover_embedded_git_repos(root, include_third_party=True)

        self.assertEqual(embedded, ["scratch"])
    ...
    def test_discover_embedded_git_repos_can_skip_third_party(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            embedded = repo_hygiene.discover_embedded_git_repos(root, include_third_party=False)
