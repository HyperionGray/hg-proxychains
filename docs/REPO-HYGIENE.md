...
`scripts/repo_maintenance.py` is kept only as a compatibility wrapper.
...
- stale tracked/untracked artifacts (currently `egressd-starter.tar.gz`)
- unexpected embedded git repositories (outside the allowed
  `third_party/FunkyDNS` submodule gitlink)

The scanner intentionally skips `third_party/FunkyDNS/` unfinished markers by
default, because that path is managed as an external dependency.
