...
automation. `scripts/repo_maintenance.py` is a compatibility wrapper.
...
- Known stale artifacts:
  - tracked stale artifacts (reported)
  - untracked stale artifacts (reported and auto-cleaned by `clean`)
  - currently: `egressd-starter.tar.gz`
- Embedded git repositories under the repo root (excluding allowed
  third-party gitlink/submodule paths)
...
Optional deep scan including `third_party/FunkyDNS` unfinished markers and
third-party embedded git state:
...
- `1`: blocking issues found
  - `scan`: unfinished markers, stray untracked files, stale artifacts,
    or embedded git repositories
  - `clean`: unfinished markers, tracked stale artifacts, embedded git
    repositories, or any removable clutter that could not be deleted
