...
`scripts/repo_hygiene.py` is the primary scanner used by maintenance targets.
`scripts/repo_maintenance.py` remains only as a compatibility wrapper.
...
- Known stale artifacts (`egressd-starter.tar.gz`) in tracked or untracked paths
- Embedded git repositories outside valid gitdir-based submodule checkouts
...
- `1`: blocking issues found
  ...
  - `clean`: unfinished markers, stale tracked artifacts, embedded git repos, or failed cleanup (removable clutter is deleted when possible)
