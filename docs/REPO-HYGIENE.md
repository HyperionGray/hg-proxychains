...
- Unexpected embedded git repositories (nested `.git` dirs/files), excluding
  the repository root and valid submodule gitlink files.
...
- `1`: blocking issues found
  ...
  - `clean`: unfinished markers, embedded git repos, or undeleted removable clutter
...
`clean` removes only known removable clutter (for example `__pycache__` and
backup/temp files). Embedded git repositories are intentionally reported but
never auto-deleted.
