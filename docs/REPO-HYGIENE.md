...
automation and Make targets (`make maintenance` / `make maintenance-fix`).
...
- Embedded git repositories (nested `.git` directories/files) that are not
  recognized gitlinks/submodules.
...
use `--include-third-party`. The default first-party mode skips any path under
`third_party/`.
...
- `1`: blocking issues found
  ...
  - `clean`: unfinished markers or embedded git repos
    (removable clutter is deleted)
