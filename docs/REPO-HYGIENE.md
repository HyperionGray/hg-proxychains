...
Use `scripts/repo_hygiene.py` as the primary maintenance scanner/cleaner.
`scripts/repo_maintenance.py` remains available as a compatibility wrapper for
legacy invocations.
...
- Known stale artifact paths (`egressd-starter.tar.gz`)
- Embedded git repositories outside the repository root and allowed gitlinks
  (for example, accidental nested clones under `scratch/`)
...
When you do want full-repo scanning (including third-party internals and nested
third-party git state),
...
`scripts/repo_maintenance.py` delegates to `repo_hygiene.py` and is retained
