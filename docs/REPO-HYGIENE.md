...
`scripts/repo_hygiene.py` is the underlying scanner/cleaner used by scheduled maintenance.
`scripts/repo_maintenance.py` remains a compatibility wrapper (`make maintenance` / `make maintenance-fix`).
...
`scripts/repo_maintenance.py` delegates to `scripts/repo_hygiene.py`.
