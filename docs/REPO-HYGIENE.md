...
`scripts/repo_hygiene.py` is the primary scanner/cleaner. `scripts/repo_maintenance.py`
is retained as a compatibility wrapper for legacy invocations.
...
- Embedded git repositories (nested `.git` directories) outside expected gitlink files.
...
- `1`: blocking issues found
  - `scan`: unfinished markers, stray untracked files, or embedded git repositories
  - `clean`: unfinished markers, embedded git repositories, or undeletable stray paths
