...
`scripts/repo_hygiene.py` is the primary scanner/cleaner used by scheduled automation.
...
- Known stale artifact paths (`STALE_ARTIFACT_PATHS`) split by tracked vs untracked
  state in reports (currently `egressd-starter.tar.gz`)
- Embedded git repositories outside expected roots (for example accidental nested
  checkouts in scratch dirs)
...
- `1`: blocking issues found
  ...
  - `clean`: unfinished markers, tracked stale artifacts, embedded git repos, or unresolved cleanup paths
