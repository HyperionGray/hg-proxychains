...
- Unexpected embedded git repositories (nested `.git` directories/files),
  excluding gitlink submodule pointers.
...
`clean` does not remove embedded git repositories automatically; it reports them
so they can be reviewed and removed deliberately.
