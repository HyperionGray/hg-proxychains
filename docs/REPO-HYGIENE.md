...
`scripts/repo_hygiene.py` is the primary scanner used by `make maintenance`
and related targets. `scripts/repo_maintenance.py` remains available as a
...
- Known stale artifacts:
  - tracked stale paths (reported only)
  - untracked stale paths (reported and removable in `clean`)
- Unexpected embedded git repositories:
  - nested `.git` directories/files outside the repository root
  - gitlink files for legitimate submodule checkouts are ignored
...
stale artifacts, or embedded git repo findings).
