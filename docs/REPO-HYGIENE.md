...
`scripts/repo_hygiene.py` is the primary scanner and cleaner for scheduled
automation (`make maintenance` / `make maintenance-fix`).
...
- Unexpected embedded git repositories (nested `.git` directories) outside the
