...
`scripts/repo_hygiene.py` is the primary maintenance scanner used by
scheduled automation and local maintenance targets.
...
- Unexpected embedded git repositories (outside the allowed
  `third_party/FunkyDNS` submodule root)
...
- `1`: blocking issues found
  - `scan`: unfinished markers, stray untracked files, stale artifacts, or embedded repos
  - `clean`: issues remain after cleanup (for example unfinished markers, embedded repos, or tracked stale artifacts)
