...
and by `make maintenance` / `make maintenance-fix`.
...
- Known stale artifact paths (tracked or untracked):
  - `egressd-starter.tar.gz`
- Embedded git repositories under the repo root that are not expected gitlinks
  (for example, nested accidental clones)
...
When you do want full-repo scanning (including nested third-party marker/stray
checks and embedded git state), use `--include-third-party`.
...
- `1`: blocking issues found
  ...
  - `clean`: unfinished markers, tracked stale artifacts, embedded git repos, or cleanup failures
...
to `repo_hygiene.py` for scan/clean behavior.
