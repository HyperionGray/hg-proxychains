...
By default, scans are first-party only (`third_party/` is excluded). Add
`--include-third-party` for deep scans when reviewing dependency trees.
...
# Include third_party marker and embedded-git scan explicitly
...
- Embedded git repositories are reported in both text and JSON output.
- Embedded git repositories are reported but never auto-removed by `clean`.
- Without `clean`, exit code is `1` when any issues are found.
