...
Use `scripts/repo_hygiene.py` directly for full control over checks, cleanup,
...
By default, `repo_hygiene.py` scans first-party paths only; pass
`--include-third-party` to include `third_party/FunkyDNS`.
...
# Refresh baseline suppressions for known third-party debt
