...
For deep full-repository scans, opt in with `--include-third-party`.
...
- `clean` removes backup files, stray `__pycache__/` directories, and known stale untracked artifacts.
...
- `scan` exits `1` when unfinished markers, stray files, stale artifacts, or embedded git repositories are found.
- `clean` exits `0` only when no issues remain after cleanup; tracked stale artifacts and embedded git repositories remain blocking.
