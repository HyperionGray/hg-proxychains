...
By default, marker scanning skips `third_party/FunkyDNS` so first-party
automation is not blocked by external dependency internals. Use
`--include-third-party` when you explicitly want a full dependency scan.
...
- `--fix` removes backup files, stray `__pycache__/` directories, and known
