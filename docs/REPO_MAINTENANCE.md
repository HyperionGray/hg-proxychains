...
For dependency audits, enable `--include-third-party`.
...
# Prune stale baseline entries that no longer exist in tracked code
...
- Baseline drift is reported as `stale_baseline_entries`; use `baseline-prune` to clean it.
