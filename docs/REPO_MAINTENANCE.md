...
By default, marker scanning is first-party only.
Use `--include-third-party` when you want to include tracked files in
...
# Write/update baseline file from current findings
...
- `--baseline-file` customizes the baseline path (default: `.repo-hygiene-baseline.json`).
