...
By default, marker scanning stays in first-party code (`--no-include-third-party`).
Use `--include-third-party` only when you intentionally need to inspect dependency internals.
...
# Prune stale baseline entries that no longer match tracked findings
...
- `baseline-prune` removes stale and duplicate baseline entries while keeping active suppressions.
