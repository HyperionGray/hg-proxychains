...
By default, marker scanning is first-party only (`--no-include-third-party`).
Use `--include-third-party` when you intentionally want to scan
`third_party/FunkyDNS` internals as well.
...
# Write a persisted JSON report file (relative to --repo-root)
python3 scripts/repo_hygiene.py scan --repo-root . --output docs/reports/hygiene-scan.json
