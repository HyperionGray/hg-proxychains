...
- Known stale artifacts (currently `egressd-starter.tar.gz`, reported as tracked vs untracked)
...
- `--fix` removes backup files, stray cache directories, and known stale
  untracked artifacts.
...
- Tracked stale artifacts are reported as blocking issues and never removed by
  `--fix`.
